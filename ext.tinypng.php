...
class Way2enjoy_ext
{
    public $name = 'Way2ENJOY';
    public $version = '3.0.0';
    public $description = 'Optimize your images for performance!';
    public $settings_exist = 'y';
    public $docs_url = 'https://github.com/way2enjoy2/expression_engine/';
    ...
    public function save_settings()
    {
        ...
        ee()->lang->loadfile('way2enjoy');

        ee('CP/Alert')->makeInline('way2enjoy-save')
            ->asSuccess()
            ->withTitle(lang('message_success'))
            ->addToBody(lang('preferences_updated'))
            ->defer();

        ee()->functions->redirect(ee('CP/URL')->make('addons/settings/way2enjoy'));
    }

    /**
     * Method that is called by the hook to run Way2ENJOY on the image
     */
    public function way2e($file_id, $data)
    {
    ...
    /**
     * Send the image to Way2ENJOY to get compressed
     */
    private function sendImage($file)
    {
        $request = curl_init();
        curl_setopt_array($request, array(
            CURLOPT_URL            => 'https://way2enjoy.com/modules/compress-png/way2enjoy-cli2.php',
    ...
    /**
     * Download the compressed image from Way2ENJOY
     */
    private function downloadImage($response, $request, $file)
    {
   //     if (curl_getinfo($request, CURLINFO_HTTP_CODE) === 201) {
	      if (curl_getinfo($request, CURLINFO_HTTP_CODE) >= 101) {
